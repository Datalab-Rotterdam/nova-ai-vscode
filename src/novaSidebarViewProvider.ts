...
import { COMMAND_OPEN_CHAT, COMMAND_REFRESH_MODELS, COMMAND_SIGN_IN, COMMAND_SIGN_OUT, NOVA_SIDEBAR_VIEW_ID } from './constants';
...
export class NovaSidebarViewProvider implements vscode.WebviewViewProvider {
  ...
  public async refresh(): Promise<void> {
    ...
    const logoUri = this.view.webview.asWebviewUri(
      vscode.Uri.joinPath(this.extensionUri, 'resources', 'icons', 'icon-192-rounded.png')
    ).toString();
    const backgroundVideoUri = this.view.webview.asWebviewUri(
      vscode.Uri.joinPath(this.extensionUri, 'resources', 'background.mp4')
    ).toString();
    const state: SidebarRenderState = { snapshot, preferredModel, logoUri, backgroundVideoUri };
